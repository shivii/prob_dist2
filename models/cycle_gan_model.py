...
class CycleGANModel(BaseModel):
    ...
    def backward_G(self, opt):
        ...
        if opt.advloss == 0:
            ...
            self.loss_G_A = div_G_A + self.criterionGAN(self.netD_A(self.fake_B), True) * opt.gen_coeff
            #print("generator loss fake_B", self.loss_G_A)
            # GAN loss D_B(G_B(B))
            div_G_B = self.get_adv.adv_loss(self.netD_B(self.fake_A), self.netD_B(self.real_B)) 
            self.loss_G_B = div_G_B + self.criterionGAN(self.netD_B(self.fake_A), True) * opt.gen_coeff

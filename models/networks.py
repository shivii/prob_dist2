...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    elif netD == 'attention':  # default PatchGAN classifier with attention
...
class UnetSkipConnectionBlockWithAttention(nn.Module):
    """Defines the Unet submodule with skip connection.
        X -------------------identity----------------------
        |-- downsampling -- |submodule| -- upsampling --|
    """
    ...
    def __init__(self, outer_nc, inner_nc, attn_q_div, attn_k_div, input_nc=None,
        ...
        Parameters:
            ...
            use_dropout (bool)  -- if use dropout layers.
        """
        super(UnetSkipConnectionBlockWithAttention, self).__init__()
        self.outermost = outermost
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
